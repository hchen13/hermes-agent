...
def _supports_text_verbosity(model: str) -> bool:
    try:
        ...
        return model_supports_text_verbosity(model)
    except Exception:
        return False


def _prepare_response_request_overrides(
    model: str,
    ...
    *,
    ...
    text_verbosity = _normalize_text_verbosity(overrides.pop("text_verbosity", None))
    if text_verbosity and not is_github_responses and _supports_text_verbosity(model):
        text_config = overrides.get("text")
        normalized_text = dict(text_config) if isinstance(text_config, dict) else {}
        normalized_text.setdefault("verbosity", text_verbosity)
        overrides["text"] = normalized_text
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        request_overrides = _prepare_response_request_overrides(
            model,
