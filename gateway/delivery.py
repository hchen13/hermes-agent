...
def resolve_delivery_transport(
    ...
    *,
    account_adapters: Optional[Dict[tuple[Platform, Optional[str]], Any]] = None,
    ...
    if normalized_account_id:
        native = (account_adapters or {}).get((platform, normalized_account_id))
    else:
        native = live_adapters.get(platform)
    try:
        instance_getter = vars(config).get("get_platform_config")
    except Exception:
        instance_getter = None
    class_getter = getattr(type(config), "get_platform_config", None)
    getter = instance_getter if callable(instance_getter) else class_getter
    if callable(getter):
        try:
            native_config = getter(
                platform,
                account_id=normalized_account_id,
            ) if getter is instance_getter else getter(
                config,
                platform,
                account_id=normalized_account_id,
            )
        except TypeError:
            native_config = getter(platform) if getter is instance_getter else getter(config, platform)
    else:
        ...
        if target_lower == "origin":
            if origin:
                return cls(
                    ...
                    account_id=getattr(origin, "account_id", None),
...
class DeliveryRouter:
    ...
    def __init__(
        ...
        self.account_adapters = account_adapters or {}
    ...
    async def _deliver_to_platform(
        ...
        if not target.chat_id:
            ...
            if home:
                target = DeliveryTarget(
                    ...
                    thread_id=target.thread_id or home.thread_id,
        ...
        transport = resolve_delivery_transport(
            ...
            self.config,
            self.adapters,
            account_adapters=self.account_adapters,
            account_id=target.account_id,
        )
