...
class PlatformConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PlatformConfig":
        ...
        # schemas can evolve (for example Feishu ``accounts:`` blocks) without
        # forcing a core dataclass change first.
...
_PLATFORM_CONNECTED_CHECKERS: dict[Platform, Callable[[PlatformConfig], bool]] = {
    ...
    Platform.FEISHU: lambda cfg: bool(
        ...
        or any(
            ...
            for account_cfg in (cfg.extra.get("accounts", {}) or {}).values()
        )
    ),
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        for platform_name, platform_data in data.get("platforms", {}).items():
            try:
                ...
                if isinstance(platform_data, dict) and isinstance(platform_data.get("home_channel"), dict):
                    platform_data = dict(platform_data)
                    platform_data["home_channel"] = dict(platform_data["home_channel"])
                    platform_data["home_channel"].setdefault("platform", platform.value)
    ...
    def get_notice_delivery(
        ...
        if platform:
            if account_id:
                ...
                if account_cfg:
                    extra = account_cfg.extra if isinstance(account_cfg.extra, dict) else {}
                    if "notice_delivery" in extra:
                        return _normalize_notice_delivery(
                            extra.get("notice_delivery"),
                            "public",
                        )
            platform_cfg = self.platforms.get(platform)
    ...
    *,
    ...
    return PlatformConfig(
        ...
        gateway_restart_notification=_coerce_bool(
            account_block.get("gateway_restart_notification"),
            base_config.gateway_restart_notification,
        ),
        typing_indicator=_coerce_bool(
            account_block.get("typing_indicator"),
            base_config.typing_indicator,
        ),
        extra=merged_extra,
    )
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    feishu_has_accounts = bool(
        feishu_cfg
        and isinstance(feishu_cfg.extra, dict)
        and isinstance(feishu_cfg.extra.get("accounts"), dict)
        and feishu_cfg.extra.get("accounts")
    )
