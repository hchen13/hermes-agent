...
class PlatformConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PlatformConfig":
        ...
        # schemas can evolve (e.g. multi-account ``accounts:`` blocks) without
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    # FEISHU_HOME_CHANNEL is honoured for both single-account (handled
    # above) and multi-account-only configurations.  When the platform
    # is configured purely via ``accounts:`` (no top-level app_id), we
    # still want the home channel applied with the appropriate
    # ``account_id`` so cron and tool deliveries route correctly.
    feishu_home = os.getenv("FEISHU_HOME_CHANNEL")
    if feishu_home and Platform.FEISHU in config.platforms:
        feishu_home_account_id = None
        try:
