...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        try:
            ...
            if getattr(event.source, "account_id", None):
                dedup_data["account_id"] = event.source.account_id
    ...
    async def _handle_set_home_command(self, event: MessageEvent) -> str:
        ...
        home = HomeChannel(
            ...
            account_id=(
                ...
                if source.platform == Platform.FEISHU and source.account_id
                else None
            ),
            user_id=(
                str(source.user_id)
        ...
        if source.platform == Platform.FEISHU:
            raw_accounts = platform_config.extra.get("accounts", {})
            if isinstance(raw_accounts, dict):
                for raw_account_config in raw_accounts.values():
                    if isinstance(raw_account_config, dict):
                        raw_account_config.pop("home_channel", None)
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        if getattr(event.source, "account_id", None):
            pending["account_id"] = event.source.account_id
        if event.source.thread_id:
