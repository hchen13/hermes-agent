...
# than post's, so the pattern is intentionally permissive.
...
class FeishuAdapterSettings:
    ...
    # Outbound formatting strategy:
    #   "post": legacy {tag:"md"} post messages.
    #   "card": always emit schema 2.0 interactive cards for markdown content.
    #   "auto": emit cards only when content needs the richer renderer.
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # message_id -> msg_type used for edit routing. Cards use patch; text/post use update.
        self._sent_msg_types: "OrderedDict[str, str]" = OrderedDict()
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        if require_mention is None and not account_scoped:
        ...
        return FeishuAdapterSettings(
            app_id=_resolve_env_reference(extra.get("app_id"))
            or str("" if account_scoped else os.getenv("FEISHU_APP_ID", "")).strip(),
            app_secret=_resolve_env_reference(extra.get("app_secret"))
            or str("" if account_scoped else _get_scoped_secret("FEISHU_APP_SECRET", "")).strip(),
            domain_name=(
            ...
            encrypt_key=_resolve_env_reference(extra.get("encrypt_key"))
            or str("" if account_scoped else _get_scoped_secret("FEISHU_ENCRYPT_KEY", "")).strip(),
            verification_token=_resolve_env_reference(extra.get("verification_token"))
            or str("" if account_scoped else _get_scoped_secret("FEISHU_VERIFICATION_TOKEN", "")).strip(),
            group_policy=str(
            ...
            bot_open_id=_resolve_env_reference(extra.get("bot_open_id"))
            or str("" if account_scoped else os.getenv("FEISHU_BOT_OPEN_ID", "")).strip(),
            bot_user_id=_resolve_env_reference(extra.get("bot_user_id"))
            or str("" if account_scoped else os.getenv("FEISHU_BOT_USER_ID", "")).strip(),
    ...
    def _resolve_edit_msg_type(self, message_id: str, new_msg_type: str) -> str:
        return self._sent_msg_types.get(message_id) or new_msg_type
    ...
    def _maybe_fallback_to_text_on_exception(
        ...
        *,
        ...
        if msg_type == "post" and _POST_CONTENT_INVALID_RE.search(text):
            ...
            return None
        return self._send_text_fallback(chunk=chunk, chat_id=chat_id, reply_to=reply_to, metadata=metadata)
    ...
    async def edit_message(
        ...
        *,
        ...
        try:
            ...
            response = await self._dispatch_edit(message_id=message_id, msg_type=new_msg_type, payload=payload)
            result = self._finalize_send_result(response, "update failed")
            if not result.success and self._edit_should_fallback(msg_type=new_msg_type, error=result.error or ""):
                response = await self._dispatch_edit_text_fallback(
                    message_id=message_id,
    ...
    async def _dispatch_edit(self, *, message_id: str, msg_type: str, payload: str) -> Any:
        if _is_card_msg_type(msg_type):
            ...
            return await self._run_blocking(self._client.im.v1.message.patch, request)
        body = self._build_update_message_body(msg_type=msg_type, content=payload)
        request = self._build_update_message_request(message_id=message_id, request_body=body)
        return await self._run_blocking(self._client.im.v1.message.update, request)
    ...
    def _edit_should_fallback(*, msg_type: str, error: str) -> bool:
        if msg_type == "post":
            return bool(_POST_CONTENT_INVALID_RE.search(error))
        if msg_type == "interactive":
            return bool(_CARD_CONTENT_INVALID_RE.search(error))
        return False
    ...
    async def send_image_file(
        ...
        **kwargs,
        ...
        try:
            ...
            if caption:
                msg_type, payload = self._build_image_caption_payload(caption=caption, image_key=image_key)
    ...
    async def _hydrate_bot_identity(self) -> None:
        ...
        # uses via probe_bot().
        try:
            req = self._build_bot_info_request()
            resp = await self._run_blocking(self._client.request, req)
            content = getattr(getattr(resp, "raw", None), "content", None)
            if content:
                payload = json.loads(content)
    ...
    def _build_outbound_payload(
        ...
        *,
        ...
        if not prefer_post and not self._has_markdown(prepared):
    ...
    def _build_image_caption_payload(self, *, caption: str, image_key: str) -> tuple[str, str]:
        prepared_caption = _convert_markdown_tables(caption, self._markdown_table_mode)
        if self._should_use_card(prepared_caption):
            return "interactive", _build_image_card_payload(caption=prepared_caption, image_key=image_key)
        return (
            "post",
    ...
    async def _send_raw_message(
        ...
        *,
        ...
        if _thread_id:
            raise ValueError(
                "Feishu thread delivery requires reply_to or metadata.reply_to_message_id; "
                "message.create does not support receive_id_type=thread_id"
            )
        ...
        *,
        ...
        for attempt in range(_FEISHU_SEND_ATTEMPTS):
            try:
                ...
                if (
                    msg_type == "post"
                    and _POST_CONTENT_INVALID_RE.search(str(exc))
                ) or (
                    msg_type == "interactive"
                    and _CARD_CONTENT_INVALID_RE.search(str(exc))
                ):
                    raise
    ...
    @staticmethod
    def _build_bot_info_request() -> Any:
        if all(name in globals() for name in ("BaseRequest", "HttpMethod", "AccessTokenType")):
            return (
                BaseRequest.builder()
                .http_method(HttpMethod.GET)
...
def _is_connected(config) -> bool:
    ...
    if isinstance(accounts, dict):
        return any(
            ...
            and bool(_resolve_env_reference(account_cfg.get("app_id")))
