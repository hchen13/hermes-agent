...
def test_turn_route_injects_text_verbosity_for_supported_models():
    runner = _make_runner()
    runner._text_verbosity = "low"
    runtime_kwargs = {
        "api_key": "***",
...
async def test_run_agent_passes_priority_processing_to_gateway_agent(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        gateway_run,
        "_load_gateway_runtime_config",
        lambda: {"agent": {"service_tier": "fast", "text_verbosity": "low"}},
    )
    monkeypatch.setattr(gateway_run, "_resolve_gateway_model", lambda config=None: "gpt-5.4")
    monkeypatch.setattr(
        gateway_run,
