...
def model_supports_text_verbosity(model_id: Optional[str]) -> bool:
    """Return whether a model accepts OpenAI's GPT-5 verbosity control."""
    raw = _strip_vendor_prefix(str(model_id or ""))
    base = raw.split(":")[0]
    return base.startswith("gpt-5") and "codex" not in base


def resolve_text_verbosity_overrides(
    model_id: Optional[str],
    verbosity: Optional[str],
) -> dict[str, Any] | None:
    """Return a transport-neutral GPT-5 verbosity override."""
    value = str(verbosity or "").strip().lower()
    if value not in {"low", "medium", "high"}:
        return None
    if not model_supports_text_verbosity(model_id):
        return None
    return {"text_verbosity": value}
...
def resolve_model_request_overrides(
    ...
    *,
    ...
    """Resolve all model-scoped request overrides for one turn."""
    overrides: dict[str, Any] = {}
    if service_tier:
        overrides.update(resolve_fast_mode_overrides(model_id) or {})
    overrides.update(resolve_text_verbosity_overrides(model_id, text_verbosity) or {})
