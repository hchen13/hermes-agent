...
def _target_matches_origin(
    ...
    if str(origin.get("platform", "")).lower() != str(platform_name).lower():
        return False
    origin_account_id = origin.get("account_id")
    if origin_account_id and account_id and str(origin_account_id) != str(account_id):
        return False
    if origin_account_id and not account_id:
        return False
...
def _resolve_single_delivery_target(job: dict, deliver_value: str) -> Optional[dict]:
    ...
    if deliver_value == "origin":
        ...
        if "origin" in job:
            return None
        # Older jobs may not have an origin field at all. Preserve their
        # historical origin-or-home behavior, but do not use this path for new
        # jobs that explicitly recorded origin=None or an unresolvable local
        # provenance marker.
    ...
    if origin and origin.get("platform") == platform_name:
        home_target = _resolve_home_delivery_target(platform_name, origin=origin)
        if home_target:
            return home_target
        target = {
            "platform": platform_name,
...
def _resolve_home_delivery_target(platform_name: str, origin: dict | None) -> Optional[dict]:
    ...
    Environment variables keep their historical precedence. If no env home is
    present, fall back to config-backed home channels, including Feishu account
    homes selected from the origin account_id.
    """
    ...
    try:
        ...
        logger.debug(
            "Failed to resolve config-backed home channel for deliver=%s",
            platform_name,
            exc_info=True,
        )
    return None
...
def _deliver_result(
    ...
    for target in targets:
        ...
        mirror_this_target = mirror_enabled and _target_matches_origin(
            origin, platform_name, chat_id, thread_id, account_id=account_id
        )
        ...
        if transport is not None:
            ...
            pconfig = _resolve_platform_config(
                config,
                platform,
                account_id=account_id,
            )
...
def _deliver_result_for_run_one_job(
    job,
    content,
    *,
    adapters=None,
    ...
    try:
        params = inspect.signature(_deliver_result).parameters
        accepts_account_adapters = (
            "account_adapters" in params
            or any(param.kind is inspect.Parameter.VAR_KEYWORD for param in params.values())
        )
    except (TypeError, ValueError):
        accepts_account_adapters = True
    if accepts_account_adapters:
        kwargs["account_adapters"] = account_adapters
    return _deliver_result(job, content, **kwargs)
    ...
    try:
        ...
        try:
            ...
            if should_deliver:
                ...
                try:
                    delivery_error = _deliver_result_for_run_one_job(
                        job,
                        deliver_content,
                        adapters=adapters,
                        account_adapters=account_adapters,
...
def tick(
    ...
    *,
    ...
    Args:
        ...
        account_adapters: Optional dict mapping (Platform, account_id) → adapter
