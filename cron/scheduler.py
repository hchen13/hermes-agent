...
def _get_home_target_thread_id(platform_name: str) -> Optional[str]:
    ...
    if not _env_home_target_chat_id(platform_name):
        home = _get_config_home_channel(platform_name)
        home_thread_id = getattr(home, "thread_id", None) if home is not None else None
        if home_thread_id:
...
def _resolve_home_delivery_target(platform_name: str, origin: dict | None) -> Optional[dict]:
    ...
    """
    # Resolve the legacy env mirror first. Config-backed homes are handled
    # below so their account_id provenance is not discarded.
    chat_id = _env_home_target_chat_id(platform_name)
    if chat_id:
        return {
            "platform": platform_name,
            "chat_id": chat_id,
            ...
            if should_deliver:
                ...
                try:
                    with _side_effect_fence() as owns_delivery:
                        ...
                        if account_adapters is not None:
                            delivery_kwargs["account_adapters"] = account_adapters
